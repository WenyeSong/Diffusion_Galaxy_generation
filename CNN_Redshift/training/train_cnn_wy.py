"""
CNN Redshift Predictor - adapted for 64x64 images on CSD3.
...
# ─── Paths ────────────────────────────────────────────────────────────────────
TRAIN_PATH = '/rds/user/ws452/hpc-work/lizarraga_2024/data/5x64x64_training_with_morphology.hdf5'
...
# ─── Hyperparameters ──────────────────────────────────────────────────────────
BATCH_SIZE  = 128
NUM_EPOCHS  = 50       # start with 50, can increase later
LR          = 1e-4
...
class HDF5Generator(tf.keras.utils.Sequence):
    def __init__(self, path, batch_size=128, shuffle=True, mode='train'):
        self.path       = path
    ...
    def __getitem__(self, idx):
        ...
        mags = (mags - 20.0) / 5.0   # rough standardisation around typical mag range
...
# ─── Model (64x64 version) ────────────────────────────────────────────────────
def build_model():
    # CNN branch — 5 pool layers: 64→32→16→8→4→2
    ...
    x = MaxPooling2D((2,2), data_format='channels_first')(x)   # 32×32
    x = Conv2D(64,  (3,3), activation='tanh', padding='same', data_format='channels_first')(x)
    x = MaxPooling2D((2,2), data_format='channels_first')(x)   # 16×16
    x = Conv2D(128, (3,3), activation='tanh', padding='same', data_format='channels_first')(x)
    x = MaxPooling2D((2,2), data_format='channels_first')(x)   # 8×8
    x = Conv2D(256, (3,3), activation='tanh', padding='same', data_format='channels_first')(x)
    x = MaxPooling2D((2,2), data_format='channels_first')(x)   # 4×4
    x = Conv2D(256, (3,3), activation='tanh', padding='same', data_format='channels_first')(x)
    x = MaxPooling2D((2,2), data_format='channels_first')(x)   # 2×2
    ...
    # NN branch — photometric magnitudes

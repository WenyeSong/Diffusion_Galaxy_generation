...
class EMA:   #  Exponential Moving Average, average weight to make model stable
    ...
    def register(self, model):
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                self.shadow[name] = param.data.clone().detach()  # copy all the model paras to a shadow
...
class DoubleConv(nn.Module):
    # 2 layers of convolution network, two 3*3 kernel, receptive field = 5*5, give model understanding of the general picture
...
class UNet_conditional_conv(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        if y is not None and self.label_emb is not None:
            ...
            t = t + y_emb  # Integrate label embedding with time encoding： put redshift information as condition together with time

        # Encoder
        x1 = self.inc(x)
        x2 = self.down1(x1, t)  # t = t+ y_emb
        x3 = self.down2(x2, t)
        x4 = self.down3(x3, t)
